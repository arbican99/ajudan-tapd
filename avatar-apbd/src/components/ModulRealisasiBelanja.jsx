...
import { 
  ...
  Printer, Download, FileSpreadsheet, Settings
} from 'lucide-react';
...
export default function ModulRealisasiBelanja() {
  ...
  const fetchMainData = async () => {
    ...
    try {
      ...
      if (mrekData) mrekData.forEach(item => newMrekMap.set(item.kdrek ? item.kdrek.trim() : '', item.nmrek));
  ...
  // --- HANDLER CETAK BROWSER ---
  const handlePrintPDF = () => {
    window.print();
  };

  // --- HANDLER DOWNLOAD PDF ULTRA-FAST (NATIVE JS) ---
  const handleDownloadPDF = async () => {
    ...
    try {
      const doc = new jsPDF({
        ...
        unit: 'mm',
        format: printPaperSize.toLowerCase()
      });
      ...
      doc.setFontSize(11);
      doc.text('LAPORAN REALISASI ANGGARAN', pageWidth / 2, 12, { align: 'center' });
      doc.setFontSize(9);
      doc.text(`TAHUN ANGGARAN ${selectedTahun === 'ALL' ? '2026' : selectedTahun}`, pageWidth / 2, 17, { align: 'center' });

      // Menyiapkan Data Tabel untuk AutoTable
      const tableRows = [];
      ...
      cetakReportTree.forEach((skpd, skpdIdx) => {
        ...
        tableRows.push([
          noUtama.toString(),
          skpd.kode,
          skpd.nama.toUpperCase(),
          formatAngkaIndo(skpd.anggaran),
          formatAngkaIndo(skpd.realisasi),
          hitungPersenAngka(skpd.anggaran, skpd.realisasi),
          formatAngkaIndo(sisaSkpd)
        ]);
        ...
        skpd.subunitList.forEach((sub, subIdx) => {
          ...
          if (isDinkesSubunit) {
            tableRows.push([
              noSubunit,
              sub.kode,
              sub.nama.toUpperCase(),
              formatAngkaIndo(sub.anggaran),
              formatAngkaIndo(sub.realisasi),
              hitungPersenAngka(sub.anggaran, sub.realisasi),
              formatAngkaIndo(sisaSubunit)
            ]);
          }
          ...
          sub.subgiatList.forEach((sg, sgIdx) => {
            ...
            tableRows.push([
              ...
              sg.kode,
              sg.nama,
              formatAngkaIndo(sg.anggaran),
              formatAngkaIndo(sg.realisasi),
              hitungPersenAngka(sg.anggaran, sg.realisasi),
              formatAngkaIndo(sisaSg)
            ]);
            ...
            sg.rekeningList.forEach((rk) => {
              ...
              tableRows.push([
                ...
                rk.kode,
                `  ${rk.nama}`,
                formatAngkaIndo(rk.anggaran),
                formatAngkaIndo(rk.realisasi),
                hitungPersenAngka(rk.anggaran, rk.realisasi),
                formatAngkaIndo(sisaRk)
              ]);
            });
          });
        });
      ...
      // Render Tabel Otomatis (Border Seragam 0.1pt)
      autoTable(doc, {
        startY: 22,
        head: [
          [
            { content: 'No', rowSpan: 2, styles: { halign: 'center', valalign: 'middle' } },
            { content: 'Kode', rowSpan: 2, styles: { halign: 'center', valalign: 'middle' } },
            { content: 'Keterangan', rowSpan: 2, styles: { halign: 'center', valalign: 'middle' } },
            { content: 'Usulan Perubahan APBD', rowSpan: 2, styles: { halign: 'center', valalign: 'middle' } },
            { content: 'Realisasi SPJ', colSpan: 2, styles: { halign: 'center' } },
            { content: 'Sisa APBD', rowSpan: 2, styles: { halign: 'center', valalign: 'middle' } }
          ],
          [
        ...
        styles: { 
          ...
          font: 'helvetica',
          textColor: [0, 0, 0],
          lineWidth: 0.1,             // ✅ Ketebalan border seragam untuk seluruh sel
          lineColor: [120, 120, 120]  // ✅ Warna border konsisten
        },
        headStyles: { 
      ...
      // Download PDF
      doc.save(`LAPORAN_REALISASI_${selectedTahun}_${new Date().getTime()}.pdf`);
    } catch (err) {
  ...
  // --- RENDER DINKES EXPANDING SUBROWS COMPONENT ---
  ...
  const renderCetakDataView = () => {
    return (
      <div className="space-y-6">
        ...
        {/* SETTINGS PANEL (Tidak ikut tercetak) */}
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800 pb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-mono text-sm font-black text-amber-400 uppercase tracking-widest">
                PENGATURAN CETAKAN & DOWNLOAD LAPORAN
              </h3>
            </div>
            ...
            <div className="flex flex-wrap gap-2 w-full md:w-auto">
              ...
              <button
                ...
                className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-400 to-yellow-500 hover:from-amber-300 hover:to-yellow-400 text-slate-950 font-mono text-xs font-black rounded-xl shadow-lg transition-all cursor-pointer disabled:opacity-50"
              >
                <Download size={15} />
                <span>{downloadingPdf ? 'MEMPROSES PDF...' : 'DOWNLOAD FILE PDF'}</span>
              </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 font-mono text-xs">
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <option value="wide">Lebar / Wide (20 mm)</option>
              </select>
            </div>
        ...
        {/* PREVIEW CONTAINER (Hanya ini yang tercetak) */}
        <div className="bg-slate-950 border-2 border-slate-800 rounded-2xl p-2 sm:p-4 overflow-x-auto shadow-2xl flex justify-center">
          <div 
            id="area-cetak-print"
            ref={printAreaRef}
            className="bg-white text-black font-sans shadow-2xl p-4 sm:p-6 rounded transition-all w-full"
            style={{ maxWidth: printOrientation === 'portrait' ? '210mm' : '297mm' }}
          >
            <div className="text-center font-bold font-sans uppercase mb-4 space-y-0.5 text-black">
              <h2 className="text-sm sm:text-base tracking-wide border-b-2 border-black pb-1 inline-block font-extrabold">
                LAPORAN REALISASI ANGGARAN
              </h2>
              <h3 className="text-xs sm:text-sm tracking-wide font-bold">
                TAHUN ANGGARAN {selectedTahun === 'ALL' ? '2026' : selectedTahun}
              </h3>
            </div>

            <table className="w-full print-preview-table font-sans">
              <colgroup>
                <col style={{ width: '5%' }} />
                <col style={{ width: '18%' }} />
                <col style={{ width: '27%' }} />
              ...
              <thead>
                <tr className="bg-gray-100 font-bold text-center text-black">
                  <th className="p-1.5 text-center" rowSpan={2}>No</th>
                  <th className="p-1.5 text-center" rowSpan={2}>Kode</th>
                  <th className="p-1.5 text-center" rowSpan={2}>Keterangan</th>
                  <th className="p-1.5 text-center" rowSpan={2}>Usulan Perubahan<br/>APBD</th>
                  <th className="p-1.5 text-center" colSpan={2}>Realisasi SPJ</th>
                  <th className="p-1.5 text-center" rowSpan={2}>Sisa APBD</th>
                </tr>
                <tr className="bg-gray-100 font-bold text-center text-black">
                  <th className="p-1 text-center">Rp</th>
                  <th className="p-1 text-center">%</th>
                </tr>
                <tr className="bg-gray-50 font-bold text-center text-black text-[9px]">
                  <td className="p-0.5 text-center">1</td>
                  <td className="p-0.5 text-center">2</td>
                  <td className="p-0.5 text-center">3</td>
                  <td className="p-0.5 text-center">5</td>
                  <td className="p-0.5 text-center" colSpan={2}>7</td>
                  <td className="p-0.5 text-center">8 = 5 - 7</td>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <React.Fragment key={skpdIdx}>
                      <tr className="font-bold bg-white text-black">
                        <td className="p-1 text-center">{noUtama}</td>
                        <td className="p-1 text-left">{skpd.kode}</td>
                        <td className="p-1 text-left uppercase">{skpd.nama}</td>
                        <td className="p-1 text-right">{formatAngkaIndo(skpd.anggaran)}</td>
                        <td className="p-1 text-right">{formatAngkaIndo(skpd.realisasi)}</td>
                        <td className="p-1 text-center">{hitungPersenAngka(skpd.anggaran, skpd.realisasi)}</td>
                        <td className="p-1 text-right">{formatAngkaIndo(sisaSkpd)}</td>
                      </tr>
                      ...
                      {skpd.subunitList.map((sub, subIdx) => {
                        ...
                        return (
                          <React.Fragment key={subIdx}>
                            {isDinkesSubunit && (
                              <tr className="font-bold bg-white text-black">
                                <td className="p-1 text-center">{noSubunit}</td>
                                <td className="p-1 text-left">{sub.kode}</td>
                                <td className="p-1 text-left uppercase">{sub.nama}</td>
                                <td className="p-1 text-right">{formatAngkaIndo(sub.anggaran)}</td>
                                <td className="p-1 text-right">{formatAngkaIndo(sub.realisasi)}</td>
                                <td className="p-1 text-center">{hitungPersenAngka(sub.anggaran, sub.realisasi)}</td>
                                <td className="p-1 text-right">{formatAngkaIndo(sisaSubunit)}</td>
                              </tr>
                            )}
                            ...
                            {sub.subgiatList.map((sg, sgIdx) => {
                              ...
                              return (
                                <React.Fragment key={sgIdx}>
                                  <tr className="font-bold bg-white text-black">
                                    <td className="p-1 text-center">{noSubgiat}</td>
                                    <td className="p-1 text-left">{sg.kode}</td>
                                    <td className="p-1 text-left">{sg.nama}</td>
                                    <td className="p-1 text-right">{formatAngkaIndo(sg.anggaran)}</td>
                                    <td className="p-1 text-right">{formatAngkaIndo(sg.realisasi)}</td>
                                    <td className="p-1 text-center">{hitungPersenAngka(sg.anggaran, sg.realisasi)}</td>
                                    <td className="p-1 text-right">{formatAngkaIndo(sisaSg)}</td>
                                  </tr>
                                  ...
                                  {sg.rekeningList.map((rk, rkIdx) => {
                                    ...
                                    return (
                                      <tr key={rkIdx} className="bg-white text-black">
                                        ...
                                        <td className="p-1 text-left text-gray-700">{rk.kode}</td>
                                        <td className="p-1 text-left pl-3">{rk.nama}</td>
                                        <td className="p-1 text-right">{formatAngkaIndo(rk.anggaran)}</td>
                                        <td className="p-1 text-right">{formatAngkaIndo(rk.realisasi)}</td>
                                        <td className="p-1 text-center">{hitungPersenAngka(rk.anggaran, rk.realisasi)}</td>
                                        <td className="p-1 text-right">{formatAngkaIndo(sisaRk)}</td>
                                      </tr>
                                    );
                                  })}
                                </React.Fragment>
